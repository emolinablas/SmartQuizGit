...
public class ListaSubAreas extends Activity {
	...
	private Boolean verificaContestadas() {
		int pendientes = 0;
		//int contador = 0;
		System.out.println(getResult().getSubArea()[0].getEstado());
		...
		// TODO Auto-generated method stub 
			...
			setSimpleAdapter( 
		         	new SimpleAdapter(this, 
		         					  ...
		         					  R.layout.fila_lista_sub_areas,
		         					  new String[] {"codigo",
		         									"subarea",
		         									"estado"}, 
		         					  new int[] {R.id.fila_lista_sub_areas_codigo,
		         ...
		         getSubAreaListView().setOnItemClickListener(new OnItemClickListener() {
		             public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		              ...
		              //  getMyDialog().AlertDialog(ListaSubAreas.this, "AVISO", "NO SE HA SUPERVISADO");
		                	try{
		               irQuizes(codigo);
	...
	protected void irQuizes(String codigo) {
		// TODO Auto-generated method stub
if (getConnectState().isConnectedToInternet(ListaSubAreas.this)){
			
	SubArea resulta = new SubArea();
	resulta = getRequestWS().indicatorList(codigo);
	if(resulta.getRespuesta() != null){
	//Answer answer = new Answer();
	/*	
		System.out.println("ESTAS SON LAS PREGUNTAS QUE RESULTAN....");
		System.out.println(resulta.getRespuesta()[0].getIndicator()[0].getPregunta());
		System.out.println(resulta.getRespuesta()[0].getIndicator()[1].getPregunta());
		System.out.println(resulta.getRespuesta()[0].getIndicator()[2].getPregunta());
		System.out.println(resulta.getRespuesta()[1].getIndicator()[0].getPregunta());
		System.out.println(resulta.getRespuesta()[1].getIndicator()[1].getPregunta());
		System.out.println(resulta.getRespuesta()[1].getIndicator()[2].getPregunta());
		*/
	...
	private Answer parseaQuizes(SubArea subarea) {// metodo que convierte el array de quizes a uno solo.
		...
		for(int k=0; k<tamanoQuizes; k++)
		{
			...
			System.out.println("esta es la cantidad de indicadores " + tamanoIndicatores);
		}
		...
		for(int i = 0; i < tamanoQuizes; i++)
		{
			...
			for(int j = 0; j < tamanoPreguntas; j++)
			{
				...
				System.out.println("pregunta que viene ->" + subarea.getRespuesta()[i].getIndicator()[j].getPregunta());
				
				indicatores[contadorIndicatores] = indicatorTemp;
				System.out.println("preguntas tem-> " + indicatores[contadorIndicatores].getPregunta());
				
				contadorIndicatores++;
			//	System.out.println("CONTADOR INDICADORES " + contadorIndicatores);
			}
...
public ArrayList<HashMap<String, String>> ListaSubAreas (SubArea[] subarea){
		...
		for (int i=0; i<subarea.length; i++){
	        ...
	        System.out.println(subarea[i].getEstado());
